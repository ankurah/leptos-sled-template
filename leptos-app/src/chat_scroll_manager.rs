...
#[derive(Clone)]
pub struct ChatScrollManager(Arc<Inner>);
...
impl ChatScrollManager {
    pub fn new(room_id: String, notification_manager: NotificationManager) -> Self {
        ...
        let metrics = Mut::new(ScrollMetrics {
            top_gap: 0.0,
            bottom_gap: 0.0,
            min_buffer: 0.0,
            step_back: 0.0,
        ...
        let messages = ctx()
            .query::<MessageView>(predicate.as_str())
            .expect("failed to create MessageView LiveQuery");
    ...
    pub fn set_live_mode(&self) {
        ...
        *self.0.last_continuation_key.borrow_mut() = None;
        ...
        self.0
            .notification_manager
            .set_active_room(Some(self.0.room_id.clone()));
    ...
    pub fn items(&self) -> Vec<MessageView> {
        ...
        if self.0.mode.peek() != ScrollMode::Forward {
    ...
    pub fn bind_container(&self, container: Option<HtmlDivElement>) {
        ...
        if let Some(old_container) = current {
            if let Some(closure) = self.0.scroll_closure.borrow_mut().take() {
                let _ = old_container.remove_event_listener_with_callback(
                    "scroll",
        ...
        *self.0.container.borrow_mut() = container.clone();
        ...
        if let Some(new_container) = container {
            *self.0.last_scroll_top.borrow_mut() = new_container.scroll_top() as f64;
            ...
            // Add event listeners
            let _ = new_container.add_event_listener_with_callback(
                "scroll",
    ...
    fn get_thresholds(&self) -> (f64, f64) {
        ...
        (
            self.0.min_buffer_size * window_px,
            self.0.continuation_step_back * window_px,
        )
    }
    ...
    fn get_continuation_anchor(
        &self,
        direction: LoadingDirection,
        message_list: &[MessageView],
    ) -> Option<(HtmlElement, MessageView)> {
        let container = self.0.container.borrow();
        ...
        if is_backward {
            ...
            for msg in message_list.iter().rev() {
                ...
                if (el.offset_top() as f64 + el.offset_height() as f64) <= target_pos {
                    tracing::info!(
                        "getContinuationAnchor backward: timestamp={}",
            ...
            for msg in message_list.iter() {
                ...
                if el.offset_top() as f64 >= target_pos {
                    tracing::info!(
                        "getContinuationAnchor forward: timestamp={}",
                        msg.timestamp().unwrap_or(0)
                    );
                    return Some((el, msg.clone()));
                }
        ...
        *self.0.last_scroll_top.borrow_mut() = scroll_top;
        ...
        if *self.0.user_scrolling.borrow() {
            *self.0.user_scrolling.borrow_mut() = false;
            ...
            if scroll_delta < 0.0
                && scroll_top < min_buffer
                && !self.at_earliest()
                && self.0.loading.peek().is_none()
            {
